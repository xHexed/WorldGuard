...
public class WorldGuardCommands {
    ...
    public WorldGuardCommands(final WorldGuard worldGuard) {
        this.worldGuard = worldGuard;
    }

    @Command(aliases = "version", desc = "Get the WorldGuard version", max = 0)
    public void version(final CommandContext args, final Actor sender) {
    ...
    public void reload(final CommandContext args, final Actor sender) throws CommandException {
        // TODO: This is subject to a race condition, but at least other commands are not being processed concurrently
        final List<Task<?>> tasks = WorldGuard.getInstance().getSupervisor().getTasks();
        if (!tasks.isEmpty()) {
    ...
    public void report(final CommandContext args, final Actor sender) throws CommandException, AuthorizationException {
        final ReportList report = new ReportList("Report");
        ...
        try {
            final File dest = new File(worldGuard.getPlatform().getConfigDir().toFile(), "report.txt");
    ...
    public void profile(final CommandContext args, final Actor sender) throws CommandException, AuthorizationException {
        final Predicate<ThreadInfo> threadFilter;
        final String threadName = args.getFlag('t');
        ...
        synchronized (this) {
            ...
            final SamplerBuilder builder = new SamplerBuilder();
        ...
        Futures.addCallback(sampler.getFuture(), new FutureCallback<Sampler>() {
            @Override
            public void onSuccess(final Sampler result) {
                final String output = result.toString();

                try {
                    final File dest = new File(worldGuard.getPlatform().getConfigDir().toFile(), "profile.txt");
    ...
    @CommandPermissions("worldguard.profile")
    public void stopProfile(final CommandContext args, final Actor sender) throws CommandException {
        synchronized (this) {
            if (activeSampler == null) {
    ...
    public void flushStates(final CommandContext args, final Actor sender) throws CommandException {
        if (args.argsLength() == 0) {
            WorldGuard.getInstance().getPlatform().getSessionManager().resetAllStates();
            sender.print("Cleared all states.");
        }
        else {
            final LocalPlayer player = worldGuard.getPlatform().getMatcher().matchSinglePlayer(sender, args.getString(0));
            if (player != null) {
    ...
    public void listRunningTasks(final CommandContext args, final Actor sender) {
        final List<Task<?>> tasks = WorldGuard.getInstance().getSupervisor().getTasks();
        ...
        if (tasks.isEmpty()) {
            ...
            final MessageBox builder = new MessageBox("Running Tasks", new TextComponentProducer());
    ...
    public void debug(final CommandContext args, final Actor sender) {}
